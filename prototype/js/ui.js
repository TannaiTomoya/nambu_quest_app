  ...
  if (state.isTicket) {
    ...
    document.getElementById("chest-text").innerHTML =
      ...
      "<div class='ticket-card'>" +
        ...
        "<a class='ticket-link' href='" + ANALYTICS.ticketHref(state.ticket.destinationId) +
          "' data-destination-id='" + state.ticket.destinationId +
...
function renderResult() {
  ...
  document.getElementById("r-item").innerHTML = state.isTicket
    ? "🎟️ <a href='" + ANALYTICS.ticketHref(state.ticket.destinationId) +
      "' data-destination-id='" + state.ticket.destinationId +
      "' target='_blank' rel='noopener' style='color:#ffd45c'>" +
      state.item + "</a>"
    : state.item;
